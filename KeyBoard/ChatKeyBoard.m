...
@interface ChatKeyBoard () <ChatToolBarDelegate, FacePanelDelegate, MorePannelDelegate,OfficialAccountToolbarDelegate>
{
...
+ (instancetype)keyBoardWithNavgationBarTranslucent:(BOOL)translucent
{
    ...
    if (translucent) {
        ...
        frame = CGRectMake(0, kScreenHeight - kChatToolBarHeight - kNavBarHeight, kScreenWidth, kChatKeyBoardHeight);
    }
...
- (void)keyBoardWillChangeFrame:(NSNotification *)notification
{
    ...
    if (self.chatToolBar.faceSelected)
    {
        [UIView animateWithDuration:0.25 delay:0 options:UIViewAnimationOptionCurveLinear animations:^{
            ...
            if (KISIphoneX) {
                self.facePanel.frame = CGRectMake(0, CGRectGetHeight(self.frame)-kFacePanelHeight-34, CGRectGetWidth(self.frame), kFacePanelHeight);
            }
    ...
    else if (self.chatToolBar.moreFuncSelected)
    {
        [UIView animateWithDuration:0.25 delay:0 options:UIViewAnimationOptionCurveLinear animations:^{
            ...
            if (KISIphoneX) {
                self.morePanel.frame = CGRectMake(0, CGRectGetHeight(self.frame)-kFacePanelHeight-34, CGRectGetWidth(self.frame), kFacePanelHeight);
            }
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary<NSString *,id> *)change context:(void *)context
{
    if (object == self && [keyPath isEqualToString:@"self.chatToolBar.frame"]) {
        ...
        if (KISIphoneX) {
            self.facePanel.frame = CGRectMake(0, CGRectGetHeight(self.frame)-kFacePanelHeight-34, CGRectGetWidth(self.frame), kFacePanelHeight);
            self.morePanel.frame = CGRectMake(0, CGRectGetHeight(self.frame)-kMorePanelHeight-34, CGRectGetWidth(self.frame), kMorePanelHeight);
        }
        [self updateAssociateTableViewFrame];
    }
}
...
- (void)chatToolBar:(ChatToolBar *)toolBar voiceBtnPressed:(BOOL)select keyBoardState:(BOOL)change
{
    if (select && change == NO) {
        ...
        [UIView animateWithDuration:0.25 animations:^{
            ...
            if (!self.facePanel.hidden) {
                self.facePanel.frame = CGRectMake(0, CGRectGetHeight(self.frame) - kFacePanelHeight, CGRectGetWidth(self.frame), kFacePanelHeight);
            }
            if (!self.morePanel.hidden) {
                self.morePanel.frame = CGRectMake(0, CGRectGetHeight(self.frame) - kFacePanelHeight, CGRectGetWidth(self.frame), kFacePanelHeight);
            }
            [self updateAssociateTableViewFrame];
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(chatKeyBoard:didSelectBarButton:)]) {
        [self.delegate chatKeyBoard:self didSelectBarButton:self.chatToolBar.voiceBtn];
    }
}
...
- (void)chatToolBar:(ChatToolBar *)toolBar faceBtnPressed:(BOOL)select keyBoardState:(BOOL)change
{
    if (select && change == NO)
    {
        ...
        [UIView animateWithDuration:0.25 animations:^{
            ...
            self.lastChatKeyboardY = self.frame.origin.y;
            self.frame = CGRectMake(0, [self getSuperViewH]-CGRectGetHeight(self.frame), kScreenWidth, CGRectGetHeight(self.frame));
            self.facePanel.frame = CGRectMake(0, 49, CGRectGetWidth(self.frame), kFacePanelHeight);
            if (KISIphoneX) {
                self.facePanel.frame = CGRectMake(0,  CGRectGetHeight(self.frame)-kFacePanelHeight - 34, CGRectGetWidth(self.frame), kMorePanelHeight);
            }
            self.morePanel.frame = CGRectMake(0, CGRectGetHeight(self.frame), CGRectGetWidth(self.frame), kFacePanelHeight);
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(chatKeyBoard:didSelectBarButton:)]) {
        [self.delegate chatKeyBoard:self didSelectBarButton:self.chatToolBar.faceBtn];
    }
}
...
- (void)chatToolBar:(ChatToolBar *)toolBar moreBtnPressed:(BOOL)select keyBoardState:(BOOL)change
{
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(chatKeyBoard:didSelectBarButton:)]) {
        [self.delegate chatKeyBoard:self didSelectBarButton:self.chatToolBar.moreBtn];
    }
}
- (void)chatToolBarSwitchToolBarBtnPressed:(ChatToolBar *)toolBar keyBoardState:(BOOL)change
{
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(chatKeyBoard:didSelectBarButton:)]) {
        [self.delegate chatKeyBoard:self didSelectBarButton:self.chatToolBar.switchBarBtn];
    }
}
...
- (void)chatToolBarTextView:(UITextView *)textView willChangeTextInRange:(NSRange)range replacementText:(NSString *)text {
    if([self.delegate respondsToSelector:@selector(chatKeyBoardTextView:willChangeTextInRange:replacementText:)]) {
        [self.delegate chatKeyBoardTextView:textView willChangeTextInRange:range replacementText:text];
    }
}

- (BOOL)chatToolBarTextView: (UITextView *)textView shouldDeleteTextInRange:(NSRange)range {
    BOOL retValue = NO;
    if([self.delegate respondsToSelector:@selector(chatKeyBoardTextView:shouldDeleteTextInRange:)]) {
        retValue = [self.delegate chatKeyBoardTextView:textView shouldDeleteTextInRange:range];
    }
    return retValue;
}
...
- (void)setTextViewContentText:(NSString *)content isUpBoard:(BOOL)isUp {
    [self.chatToolBar setTextViewContentText:content isUpBoard:isUp];
}
...
#pragma mark -- OfficialAccountToolbarDelegate
- (void)didSelectThemeItem:(int)themeIndex subItem:(int)subItem {
    if ([self.delegate respondsToSelector:@selector(chatKeyBoard:didSelectBottomThemItemIndex:subItem:)]) {
        [self.delegate chatKeyBoard:self didSelectBottomThemItemIndex:themeIndex subItem:subItem];
    }
}
...
- (void)setDataSource:(id<ChatKeyBoardDataSource>)dataSource
{
    ...
    if ([self.dataSource respondsToSelector:@selector(chatKeyBoardBottomItems)]) {
        NSArray<BottomItemModel *> *themeItems = [self.dataSource chatKeyBoardBottomItems];
        [self.OAtoolbar loadBottomThemeItems:themeItems];
    }
}
...
- (void)setIsFirstShowMenuItemBar:(BOOL)isFirstShowMenuItemBar {
    _isFirstShowMenuItemBar = isFirstShowMenuItemBar;
    if (isFirstShowMenuItemBar) {
        [self.chatToolBar handelSwitchBarClick:self.chatToolBar.switchBarBtn];
        self.chatToolBar.switchBarBtnLineView.hidden = YES;
    }
}
...
- (void)reloadMenuItems:(NSArray<BottomItemModel *> *) array{
    [self.OAtoolbar loadBottomThemeItems:array];
}
