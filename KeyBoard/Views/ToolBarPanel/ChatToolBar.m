...
- (void)setbarSubViewsFrame
{
    ...
    if (self.allowSwitchBar) {
        self.switchBarBtn.frame = CGRectMake(0, 1, ItemW, ItemH-1);
        [self.switchBarBtn addSubview:self.switchBarBtnLineView];
    }else {
        self.switchBarBtn.frame = CGRectZero;
    }
    
    if (self.allowVoice){
        self.voiceBtn.frame = CGRectMake(CGRectGetMaxX(self.switchBarBtn.frame), 0, ItemW, ItemH);
    }else {
...
- (UIView *)switchBarBtnLineView{
    if (!_switchBarBtnLineView) {
        _switchBarBtnLineView = [[UIView alloc] initWithFrame:CGRectMake(ItemW-0.5, 8, 0.5, ItemH-1 - 16)];
        _switchBarBtnLineView.backgroundColor = [UIColor lightGrayColor];
    }
    return _switchBarBtnLineView;
}
...
{
    if(!textView.markedTextRange && (textView.selectedRange.length == 0) && [text isEqualToString:@""]) {
        //delete backward
        if([self.delegate respondsToSelector:@selector(chatToolBarTextView:shouldDeleteTextInRange:)]) {
            BOOL retValue = [self.delegate chatToolBarTextView:textView shouldDeleteTextInRange:range];
            return retValue;
        }
    }
    ...
    //排除待确认输入(marked text)
    if(range.length > 0 || !textView.markedTextRange) {
        if([self.delegate respondsToSelector:@selector(chatToolBarTextView:willChangeTextInRange:replacementText:)]) {
            [self.delegate chatToolBarTextView:textView willChangeTextInRange:range replacementText:text];
        }
    }
    return YES;
}
...
{
    self.currentText =  content ? content : @"";
